...
public:
    ...
    action_name_(name)
  {
    ...
    block_poses_.header.frame_id = base_link;

    // Subscribe to point cloud
    point_cloud_sub_ = nh_.subscribe("/radar/RScan", 1, &PointCloudLab::cloudCallback, this);
    ...
    block_pose_pub_ = nh_.advertise< geometry_msgs::PoseArray >("/published_pose_array", 1, true);
  ...
  void cloudCallback( const sensor_msgs::PointCloud2ConstPtr& msg )
  {
    ...
    //tf_listener_.waitForTransform(std::string(base_link), cloud.header.frame_id,cloud.header.stamp, ros::Duration(1.0));
